...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pbr_kwargs=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        # Ensure ground-truth image exists for cameras that may not embed it (e.g., loaded from cameras.json)
        if (not hasattr(view, 'original_image')) or (view.original_image is None):
            try:
                from scene.utils import load_img_rgb
                assert hasattr(view, 'image_path') and view.image_path is not None, \
                    "Camera has no original_image and no image_path for lazy load"
                img_np = load_img_rgb(view.image_path)
                alpha_t = None
                if img_np.shape[-1] == 4:
                    alpha_np = img_np[..., 3]
                    img_np = img_np[..., :3]
                    alpha_t = torch.from_numpy(alpha_np).float().unsqueeze(0).clamp(0.0, 1.0).to(view.data_device)
                img_t = torch.from_numpy(img_np).float().permute(2, 0, 1).clamp(0.0, 1.0).to(view.data_device)
                view.original_image = img_t
                H, W = img_t.shape[-2], img_t.shape[-1]
                if (not hasattr(view, 'image_mask')) or (view.image_mask is None) or \
                   (tuple(view.image_mask.shape[-2:]) != (H, W)):
                    view.image_mask = alpha_t if alpha_t is not None else \
                        torch.ones((1, H, W), dtype=torch.float32, device=view.data_device)
            except Exception:
                # If anything goes wrong, let it fail later when accessing original_image
                pass
