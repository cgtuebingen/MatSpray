...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --run-mlp|--run_mlp)
            ...
            shift
            ;;
        --run-combined-mlp|--run_combined_mlp)
            RUN_COMBINED_MLP=true
            shift
            ;;
        ...
        --help|-h)
            ...
            echo "  --run-mlp                Run MLP training (separate MLPs)"
            echo "  --run-combined-mlp       Run combined MLP training (single MLP for all properties)"
...
echo "Run Combined MLP: $RUN_COMBINED_MLP"
...
# Base directories
root_dir="${ROOT_DIR:-/path/to/datasets/aria/}"
output_dir="${OUTPUT_DIR:-/path/to/results/aria/}"

# External 3DGS PLY root and iteration (per scene: .../iteration_${gs_iteration}/point_cloud.ply)
gs_ply_root="${gs_ply_root:-$output_dir}"
gs_iteration="${gs_iteration:-30000}"

list=("teapot_purple" "birdhousetoy" "birdhouse_BoatHouse" "birdhouse_LogRoof" "birdhouse_RedBlackRoof" "birdhouse_StoneHouse" "birdhouse_thatchedRoofWhiteWallFlowers" "birdhouse_YellowFlower" "airplane_yellow" "airplane_whitegold" "apartment" )
...
if [ "$RENDER_GTS" = true ]; then
    ...
    for i in "${list[@]}"; do
        ...
        "$BLENDER_BIN" --background $root_dir/blend_files/$blend_file_name.blend --python blender_scripts/blender_render_script_train.py -- --object_name $blend_file_name --output_dir $root_dir
...
echo "====== Starting model training pipeline ======"
    ...
    if [ "$RUN_BASE_3DGS" = true ]; then
        ...
        gs_ply="${gs_ply_root}/${i}/gaussian_splatting/base/point_cloud/iteration_${gs_iteration}/point_cloud.ply"
        if [ -f "$gs_ply" ]; then
            echo "====== ENABLED: Creating 3DGS checkpoint from external PLY (with RGB→normals) for $i ======"
            $PY utils/save_ckpt_from_points3d.py \
                --ply "$gs_ply" \
                -m "$output_dir/$i/3dgs" \
                -i "$gs_iteration" \
                --sh-degree 0 \
                --convert-rgb-to-normals
            dest_dir="$output_dir/$i/3dgs/point_cloud/iteration_${gs_iteration}"
            dest_ply="$dest_dir/point_cloud.ply"
            mkdir -p "$dest_dir"
            rm -f "$dest_ply"
            cp -f "$gs_ply" "$dest_ply"
            if [ ! -f "$dest_ply" ] || [ ! -s "$dest_ply" ]; then
                echo "[error] Copy failed: $dest_ply missing or empty" >&2
                exit 1
            fi
            echo "[copy] point_cloud.ply updated at $dest_ply ($(wc -c < "$dest_ply") bytes)"
        else
            echo "====== ENABLED: Running base 3DGS training from dataset for $i (no external PLY at $gs_ply) ======"
            $PY train.py --eval \
                -s $root_dir/$i/base/ \
                -m $output_dir/$i/3dgs \
    ...
    if [ "$RUN_BASE_NEILF" = true ]; then
        ...
        $PY train.py --eval \
            ...
            -c $output_dir/$i/3dgs/chkpnt${gs_iteration}.pth \
            --save_training_vis \
    ...
    if [ "$RUN_NEILF_SUPERVISION" = true ]; then
        ...
        for model_name in "${models[@]}"; do
            ...
            neilf_configs=(
                ["mlp_reviwer2_suggestion_${model_name}"]="1.0 1.0 1.0"
            )
            ...
            for neilf_name in "${!neilf_configs[@]}"; do
                ...
                for envmap in "${envmaps[@]}"; do
                    ...
                    $PY relighting.py \
                        ...
                        --capture_list "pbr_env,normal,base_color,visibility,roughness,metallic,diffuse,specular" \
                        --sample_num 800 \
                        --ply_path "$output_dir/$i/$neilf_name/point_cloud/iteration_35000/point_cloud.ply" \
                        --gt_path "$root_dir/$i/$envmap/test/images/" \
            ...
            for neilf_name in "${!neilf_configs[@]}"; do
                ...
                for envmap in "${envmaps[@]}"; do
                    ...
                    $PY relighting.py \
                        -co "$root_dir/$i/$envmap/" \
                        --video \
                        --output "$output_dir/$i/$neilf_name/$envmap/debug" \
                        --envmap_path "env_map/$envmap.exr" \
                        --capture_list "pbr_env,normal,base_color,visibility,roughness,metallic,diffuse,specular" \
                        --sample_num 800 \
                        --ply_path "$output_dir/$i/$neilf_name/point_cloud/iteration_40000/point_cloud.ply" \
    ...
    # Process each model with Combined MLP (single MLP for base_color, roughness, metallic)
    if [ "$RUN_COMBINED_MLP" = true ]; then
        echo "====== ENABLED: Processing Combined MLP NeILF for model: $model_name ======"
        for model_name in "${models[@]}"; do
            echo "Processing Combined MLP NeILF for model: $model_name"

            # Define all NeILF configurations for the current model
            declare -A neilf_configs
            neilf_configs=(
                ["combined_mlp_${model_name}"]="1.0 1.0 1.0"
            )
        ...
        echo "====== SKIPPED: Processing Combined MLP NeILF (use --run-combined-mlp to enable) ======"
    fi
