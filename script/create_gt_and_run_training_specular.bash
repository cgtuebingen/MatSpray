...
# External 3DGS PLY root and iteration (per scene: .../gaussian_splatting/base/point_cloud/iteration_${gs_iteration}/point_cloud.ply)
gs_ply_root="${gs_ply_root:-$output_dir}"
gs_iteration="${gs_iteration:-30000}"

list=("teapot_yellow" "coffee_canister" "fridge" "supply_bin" "teapot_jade" "kettle")

echo "Scenes (list): ${list[*]}"
echo "External 3DGS PLY root: $gs_ply_root (override with gs_ply_root=... before sourcing)"
echo "External 3DGS iteration: $gs_iteration (override with gs_iteration=... before sourcing)"
...
echo "====== Starting model training pipeline ======"
    ...
    if [ "$RUN_BASE_3DGS" = true ]; then
        ...
        if [ ! -f "$gs_ply" ]; then
            echo "[error] External 3DGS PLY not found for $i: $gs_ply" >&2
            echo "        Set gs_ply_root (default: output_dir) or gs_iteration (default: 30000) if your paths differ." >&2
            exit 1
        fi
        ...
        # PLY with normals for downstream readers
        dest_dir="$output_dir/$i/3dgs/point_cloud/iteration_${gs_iteration}"
        dest_ply="$dest_dir/point_cloud.ply"
        mkdir -p "$dest_dir"
        rm -f "$dest_ply"
        $PY utils/ply_sh0_to_normals_and_copy.py \
            --src "$gs_ply" \
            --dst "$dest_ply" \
            --mode rgb
        if [ ! -f "$dest_ply" ] || [ ! -s "$dest_ply" ]; then
            echo "[error] PLY conversion failed: $dest_ply missing or empty" >&2
            exit 1
        fi
        echo "[copy] point_cloud.ply with normals written to $dest_ply ($(wc -c < "$dest_ply") bytes)"
    ...
    if [ "$RUN_MLP" = true ]; then
        ...
        for model_name in "${models[@]}"; do
            ...
            declare -A neilf_configs
            neilf_configs=(
                ["mlp_softmax_BaseColor_Roughness_Metallic_median_deferred_moving_geom_${model_name}"]="0.0 0.0 0.0"
            )
