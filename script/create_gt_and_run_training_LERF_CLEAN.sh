...
# Ensure the script runs with bash (re-exec if invoked with sh or sourced)
if [ -z "$BASH_VERSION" ]; then
	# If the script is being sourced in a non-bash shell, abort with a clear message
	if ( return 0 2>/dev/null ); then
		echo "This script must be executed with bash, not sourced. Run: /usr/bin/env bash \"$0\""
		return 1
	fi
...
set -e
# Enable stricter error handling for pipelines
set -o pipefail

# Lightweight logging and error helpers
log() { printf '%s\n' "[INFO] $*"; }
die() { printf '%s\n' "[ERROR] $*" >&2; exit 1; }
...
# Default Blender binary path (can be overridden via environment variable)
...
if [ -d "$PROJECT_ROOT/Optix/build" ]; then
	cd "$PROJECT_ROOT/Optix/build"
	log "Building Optix in $(pwd) ..."
	make -j || die "Optix build failed"
	cd - >/dev/null || true
else
	log "Skipping Optix build: directory not found"
fi

# Base directories
root_dir="${ROOT_DIR:-/path/to/datasets/lerf_clean/}"
output_dir="${OUTPUT_DIR:-/path/to/results/lerf_clean/}"
list=("figurines/figurines")
...
if [ "$RENDER_GTS" = true ]; then
    ...
    for i in "${list[@]}"; do
        ...
        # Test render command
        echo "Running Blender test render for $i..."
        if [ ! -x "$BLENDER_BIN" ]; then die "Blender binary not found or not executable: $BLENDER_BIN"; fi
if [ ! -f "$root_dir/blend_files/$blend_file_name.blend" ]; then die "Blend file not found: $root_dir/blend_files/$blend_file_name.blend"; fi
"$BLENDER_BIN" --background "$root_dir/blend_files/$blend_file_name.blend" --python blender_scripts/blender_render_script_keyframes.py -- --object_name "$blend_file_name" --output_dir "$root_dir"
    ...
    if [ "$RUN_BASE_NEILF" = true ]; then
        ...
        for envmap in "${envmaps[@]}"; do
            ...
            $PY relighting.py \
                ...
                --capture_list "pbr_env,normal,base_color,visibility,roughness,metallic" \
                --sample_num 64 \
    ...
    if [ "$RUN_PROJECTED_AVERAGE" = true ]; then
        ...
        if [ "$RUN_MLP" = true ]; then
            ...
            for model_name in "${models[@]}"; do
                ...
                declare -A neilf_configs
                neilf_configs=(
                    ["mlp_softmax_BaseColor_Roughness_Metallic_${model_name}"]="0.0 0.0 0.0"
                )
