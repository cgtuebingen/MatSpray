...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, fx, fy, cx, cy, image, image_name, uid,
                 ...
                 height=None, width=None, depth=None, normal=None, image_mask=None, image_path=None):
    ...
    def set_resolution(self, width: int, height: int):
        ...
        # Recompute intrinsics and projection matrices to stay consistent with new resolution
        self.intrinsics = self.get_intrinsics()
        if self.fx is None:
            self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx,
                                                         fovY=self.FoVy).transpose(0, 1).cuda()
    ...
    def get_extrinsics(self):
        ...
        Rt[:3, :3] = self.R  # Don't transpose R - it's already in correct format
        Rt[:3, 3] = self.T
    ...
    def get_rotation(self):
        return torch.from_numpy(self.R).float().cuda()  # Don't transpose R
