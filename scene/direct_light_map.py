...
class DirectLightMap:
    ...
    def get_env(self):
        # return self.env
        env = F.softplus(self.env)
        # Sanitize NaN/Inf values to prevent silent propagation
        if not torch.isfinite(env).all():
            print(f"[WARNING] DirectLightMap.get_env contains NaN/Inf values - sanitizing")
            env = torch.nan_to_num(env, nan=0.0, posinf=1.0, neginf=0.0)
        return env
