...
namespace osc {
    ...
    struct CUDABuffer {
        ...
        void alloc_and_upload(const T &vt) {
            alloc(sizeof(T));
            upload_vec3(&vt, sizeof(T));
        }

        template<typename T>
        void alloc_and_upload_aabb(const T &vt) {
            alloc(sizeof(T));
            upload_aabb(&vt, sizeof(T));
        }
        ...
        template<typename T>
        void upload_vec3(const T *t, size_t size) {
            assert(d_ptr != nullptr);
        ...
        template<typename T>
        void upload_aabb(const T *t, size_t size) {
            assert(d_ptr != nullptr);
            assert(sizeInBytes == size);
            CUDA_CHECK(Memcpy(d_ptr, (void *)t,
                size, cudaMemcpyHostToDevice));
