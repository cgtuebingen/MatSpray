...
def load_and_transform_diffusion_renderer_image(image_path, modality='basecolor', gamma=1.8, device=None):
    """
    Load and transform DiffusionRenderer prediction images.
    
    Applies:
    - Gamma correction (img^gamma) to basecolor and metallic (not roughness)
    - Roughness is kept linear (no gamma correction)
    
    Args:
        image_path: Path to the image file
        modality: One of 'basecolor', 'roughness', 'metallic'
        gamma: Gamma value to apply as exponent (default: 1.8)
        device: Device to move tensor to (if None, keeps original device)
    
    Returns:
        torch.Tensor: Transformed image tensor in [C, H, W] format
    """
    import torchvision
    ...
    if device is not None:
        ...
        # Try to detect device from existing tensors or use CUDA if available
        if torch.cuda.is_available():
            img = img.cuda()
    
    # Handle RGBA images - take only RGB
    if img.shape[0] == 4:
        img = img[:3]
    
    # Apply gamma correction as exponent to all modalities except roughness
    # Clamp to avoid numerical issues
    img = torch.clamp(img, 0.0, 1.0)
    # Skip gamma correction for roughness (keep it linear)
    if modality == 'basecolor':
        img = torch.pow(img + 1e-7, gamma)
    
    # Validate output for NaN/Inf values
    if not torch.isfinite(img).all():
        print(f"[WARNING] load_and_transform_diffusion_renderer_image: Output contains NaN/Inf values from {image_path}")
